...
각 actionItems는 위 도구상자에서 문제에 맞는 기법을 골라 구체적 실천 행동으로 작성하고, 기법 이름을 괄호로 표기하세요. 또한 그 기법의 [근거]에 적힌 연구·학자를 "~~에 따르면" 형태로 인용해, 그 행동이 효과적인 근거를 한 구절로 밝히세요.
...
- actionItems에 그 기법의 [근거](연구·학자)가 "~~에 따르면" 형태로 인용되어 있는가?
...
--- 좋은 출력 예시 ---
{
  ...
  "actionItems": ["Dunlosky 등(2013)의 학습기법 연구에서 인출 연습이 최상위 효용으로 꼽힌 만큼, 다음 수업 도입부 5분 동안 지난 시간의 핵심 개념 하나를 학생이 먼저 떠올려 적게 한 뒤 실생활 사례 2가지로 복습하는 방식을 참고할 수 있습니다 (인출 연습/retrieval practice). 이후 같은 연구가 구체적 예시의 효과를 입증한 점을 참고해, 본 강의에서도 추상적 정의보다 예시를 먼저 제시하고 거기서 개념을 끌어내는 루틴을 시도해볼 수 있습니다 (구체적 예시 우선/concrete examples)."]
}
