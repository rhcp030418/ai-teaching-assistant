...
3문장: "~해보시면 어떨까요" 또는 "~을 시도해보시길 추천합니다" 형식으로, 다음 수업에서 바로 할 수 있는 구체적 행동 한 가지를 제안하세요. 이때 아래 도구상자에서 이 사례의 개선 축에 맞는 기법을 골라 구체적 실천 행동으로 풀고, 기법 이름을 괄호로 표기하세요. 더불어 그 기법의 [근거]에 적힌 연구·학자를 "~~에 따르면" 형태로 인용해, 그 제안이 효과적인 근거를 한 구절로 곁들이세요(도구상자에 없는 연구는 지어내지 마세요).
