...
각 item의 action은 위 도구상자에서 문제에 맞는 기법을 골라 구체적 실천 행동으로 작성하고, 기법 이름을 괄호로 표기하세요. 또한 그 기법의 [근거]에 적힌 연구·학자를 "~~에 따르면" 형태로 인용해, 그 행동이 효과적인 근거를 한 구절로 밝히세요.
  ...
  "items": [
    {
      "priority": "urgent",
      "category": "content",
      "action": "Dunlosky 등(2013)이 구체적 예시를 검증된 학습전략으로 꼽은 만큼, 다음 수업 전에 핵심 개념 3개에 대해 추상적 정의보다 먼저 제시할 실생활 예시를 각 2개씩 준비하세요 (구체적 예시 우선/concrete examples). 또한 마주어(Mazur, Harvard)의 연구에 따라 각 개념 설명 직후 객관식 개념 질문 1개로 거수 응답을 받아 이해도를 즉석 확인하세요 (개념점검질문/ConcepTest).",
