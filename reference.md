...
- 출력 시 행동 문장에 기법 이름을 괄호로 병기한다. 예: `...하세요 (동료 교수법/peer instruction)`
- 아래 "연구 근거"는 도구상자 상수(`TEACHING_TOOLBOX`)의 각 기법 줄에 `[근거: ...]`로 인라인 주입되어 있다. AI는 제안할 때 이 근거를 "~~에 따르면" 형태로 인용해 제안의 출처를 밝힌다. 예: `Dunlosky 등(2013)에 따르면 인출 연습이 ... 이므로, ...하세요 (인출 연습/retrieval practice)`. 도구상자에 없는 연구·수치는 지어내지 않는다.
