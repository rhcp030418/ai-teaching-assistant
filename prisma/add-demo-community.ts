// 시연용: 데모 학생이 만든 커뮤니티 "임의로 만든 강의"를 강의로 등록
// 가짜 교수 계정 (시연용) 생성 + 데모 학생 본인이 수강
...
async function main() {
  ...
  const fakeProf = await prisma.professor.upsert({
    where: { email: "demo_community@example.ac.kr" },
    update: {},
    create: { name: "데모 (시연용)", email: "demo_community@example.ac.kr", password: pw },
  });

  // 커뮤니티를 강의로 등록 (6각형 레이더용 둘 다 true)
  const course = await prisma.course.upsert({
    where: { id: "demo-community-course" },
    update: {},
    create: {
      id: "demo-community-course",
      name: "임의로 만든 강의",
      semester: "2026-1",
      category: "교양",
      studentCount: 5,
  ...
  console.log("교수: 데모 (시연용) / demo_community@example.ac.kr / demo1234");
  console.log("학생: 학생A (0000001) 자동 수강 등록");
